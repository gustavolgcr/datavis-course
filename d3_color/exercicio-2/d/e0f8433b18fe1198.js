  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container"], function(md,container)
{
  ...
  main.variable(observer("homicidios")).define("homicidios", ["d3"], function(d3){return(
...
<main role="main" class="container">
    <div class="row">
      <h3> Homicídios por 100.000 habitantes em Fortaleza em 2012</h3>
    </div>
...
d3.schemeOrRd[9]
  ...
  main.variable(observer("colorScale")).define("colorScale", ["d3","homicidios","rateByName","blues"], function(d3,homicidios,rateByName,blues){return(
d3.scaleQuantize()
    .domain([0, d3.max(homicidios, d => rateByName.get(d.Bairro))])
    .range(blues)
)});
...
function zoomToFeature(e) {
  ...
  main.variable(observer("info")).define("info", ["L","rateByName","map"], function(L,rateByName,map)
{
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Número de homicídios por habitante</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br />' + rateByName.get(feat.properties.NOME) + ' homicídios'
...
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
    ...
    data.forEach(function(d) {
       bairrosMap.set(d.Bairro,+((d.Homicidios*100000)/d.Populacao).toFixed(2))
        })
    return bairrosMap
  })
)});
  main.variable(observer("style")).define("style", ["colorScale","rateByName"], function(colorScale,rateByName){return(
function style(feature) {
		 return {
					...
					dashArray: '3',
					fillOpacity: 0.6,
					fillColor: colorScale(rateByName.get(feature.properties.NOME))
				};
	}
)});
